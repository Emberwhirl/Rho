...
| `plans/active-2026-08-05-macos-arm64-support-spec.md` | active; MAC1-MAC4 implementation/review complete; earlier rehearsal history preserved; Issue #6 `dev.20` installed artifact rejected; corrective live identity is `0.4.0-dev.21`; exact local dev.21 artifact verification pending | Apple Silicon macOS 14+ platform adapters, Ark/R integration, Keychain extension, additive macOS update artifact, signed DMG handoff, repository-bound rehearsal lane, and async notarization orchestration | MAC4-R3 remains closed at its review-only evidence; dev.21 local/installed acceptance remains open; MAC5 remains unauthorized; full M3 retains authoritative candidate, macOS x64, and Linux x64 gates |
...
| `release/active-0.4.0-dev.21-candidate-checklist.md` | active corrective identity; PROBLEMS-AGENT-REPAIR-3, CRED-UX4A-R2, and WS3-Q1-R1 implementation plus complete affected automation/browser behavior review pass; local artifact and installed acceptance open | sole live `0.4.0-dev.21` cross-platform candidate identity, future evidence binding, installed-acceptance ledger, and GO/NO-GO decision | exact local artifact then owner-installed/live-Provider acceptance; hosted candidate, MAC5, upload, and publication remain separately gated |
