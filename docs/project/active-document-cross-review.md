...
| `release/active-0.4.0-dev.24-candidate-checklist.md` | active replacement identity; Issue #6 R5 and Issue #9 Task Rail source implementation present; affected frontend/static/exact-browser validation pass; complete candidate matrix and artifact open | sole live `0.4.0-dev.24` cross-platform candidate identity, future evidence binding, installed-acceptance ledger, and GO/NO-GO decision | complete exact-candidate matrix and artifact only after separate authorization, then owner-installed/live-Provider acceptance; hosted candidate, MAC5, upload, and publication remain separately gated |
