...
| `plans/active-2026-08-05-macos-arm64-support-spec.md` | active; MAC1-MAC4 implementation/review complete; earlier rehearsal history preserved; Issue #6 `dev.20` installed artifact rejected; corrective live identity is `0.4.0-dev.21`; exact clean-source local unsigned dev.21 artifact verification passed | Apple Silicon macOS 14+ platform adapters, Ark/R integration, Keychain extension, additive macOS update artifact, signed DMG handoff, repository-bound rehearsal lane, and async notarization orchestration | MAC4-R3 remains closed at its review-only evidence; dev.21 owner-installed/live-Provider acceptance remains open; MAC5 remains unauthorized; full M3 retains authoritative candidate, macOS x64, and Linux x64 gates |
...
| `release/active-0.4.0-dev.21-candidate-checklist.md` | active corrective identity; PROBLEMS-AGENT-REPAIR-3, CRED-UX4A-R2, and WS3-Q1-R1 implementation, complete affected automation/browser review, and exact clean-source local unsigned arm64 artifact verification pass; installed acceptance open | sole live `0.4.0-dev.21` cross-platform candidate identity, future evidence binding, installed-acceptance ledger, and GO/NO-GO decision | owner-installed/live-Provider acceptance; hosted candidate, MAC5, upload, and publication remain separately gated |
...
| `plans/active-2026-08-07-problems-agent-repair-spec.md` | active; PROBLEMS-AGENT-REPAIR-1 historical; R2 implemented/verified but installed `dev.20` rejected; owner authorized D3/R3 PROBLEMS-AGENT-REPAIR-3 runtime correction 2026-08-08 | exact Workspace R expression diagnostics, schema-v10 project-scoped Problem ranges, bounded same-run context, and one-click read-only tool-capable Problems-to-Agent proposal flow with canonical registered runtime identity | Runs/Workspace execution own diagnostic truth; BH3 owns migration; CRED-UX4 owns route/one credential; Agent file-edit owns proposal/Accept; no unsolicited Provider call, automatic execution, approval, save, fuzzy location, or file mutation authority |
...
installed corrections now own replacement identity `0.4.0-dev.21` under the
...
The active `0.4.0-dev.21` checklist inherits these unchanged automation gates,
not their results. It corrects registered Agent runtime identity and selected
Workspace view refresh after the exact `dev.20` installed rejection, so it
requires a new exact-source validation, artifact, and installed-acceptance
ledger; no earlier run, notarization receipt, hash, or artifact is composable
with it.
