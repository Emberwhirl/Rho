...
| `release/active-0.4.0-dev.29-candidate-checklist.md` | active source-candidate contract; Issue #2 launcher/aisdk/fresh-readiness repairs implemented and exact-source validation/review pass, with PR #24 update pending | sole `0.4.0-dev.29` source/candidate identity and future exact-candidate, installed, MAC5, and publication evidence | no artifact, Draft, tag, installed acceptance, integration, or publication exists; immutable `dev.28` source evidence is non-composable; package versions and schema 12 remain unchanged |
| `release/active-0.4.0-dev.30-candidate-checklist.md` | active stacked source-candidate contract; Issue #33 implementation/review, exact identity synchronization/source validation, branch publication, Draft PR #34, and Issue reply complete | sole `0.4.0-dev.30` source/candidate identity and future exact-candidate, installed, MAC5, and publication evidence | stacks exact PR #24 head and must not merge before #24; Draft PR #34 remains open; no artifact, tag, installed acceptance, integration, or publication exists; `dev.29` evidence is non-composable; package versions and schema 12 remain unchanged |
...
| `plans/active-2026-08-10-windows-agent-r-script-launch-repair-spec.md` | active; Issue #2 launcher and aisdk-readiness repairs implemented with prior Windows acceptance; R1 fresh-readiness cache admission and exact `dev.29` validation/review pass, with installed `dev.29` acceptance and PR update pending | Windows desktop Agent R script transport through a temporary UTF-8 `.R` file plus complete-Agent aisdk readiness admission that never trusts general R/Ark cache state | Windows startup retains R discovery and optional-runtime cache ownership; Agent LLM configuration retains Provider/credential/routing and fresh readiness authority; Conversation concurrency retains turn identity, scheduling, cancellation, and persistence; no schema, frontend, focus behavior, approval, or release authority |
| `plans/active-2026-08-11-workbench-focus-stability-repair-spec.md` | active; Issue #33 ISSUE-33-INTERACTION-1 source implementation, 46 compatible frontend contracts plus adjacent checks, and post-verification review complete 2026-08-11; stacked `0.4.0-dev.30` publish authorized 2026-08-11; browser/mock interaction unavailable in the current environment and installed Windows acceptance open | interaction-safe volatile refresh, explicit focus authority, activation preservation, and user-owned reading position across Project files/tabs, Agent, Runs, Problems, Plots/Outputs, Environment/Data Viewer, Git review, editor, Console, and Logs | WP2/WP3/UX4/Agent conversation/Environment retain data, execution, polling, selection, approval, and mutation truth; Issue #15 cursor advancement, Issue #18 modal predicate, and UX-FIX4 explicit Console focus preserved with bidirectional amendments; publish branch must stack exact open PR #24 `dev.29`, synchronize `dev.30`, and remain merge-blocked until #24 lands; no schema, backend, credential, filesystem, or project authority |
...
| `plans/active-2026-08-05-system-credential-and-simple-llm-settings-spec.md` | active; CRED-UX1/2/3/4A and CRED-UX4A-R1 implementation verified; CRED-UX4A-R2 registered-runtime correction implemented; CRED-UX4A-R3 and Issue #25 R4 implementation/automation/browser/independent review, `dev.28` synchronization, and upstream PR #31 integration complete, with installed acceptance open | shared native system-credential semantics, Model settings, bounded Provider discovery, three-layer Connection/Model/Capability routing, reviewed `aisdk.providers` integration, canonical registered runtime identity, revision-bound Provider/model/optional-route/credential deletion, and selected-Provider/modal truthfulness | exact one-credential and no-fallback rules preserved; required `agent.chat` cannot be inferred or cascade-deleted; direct model deletion remains non-cascading; one Model-settings modal owns destructive confirmation; credential failure/metadata rollback remains fail-closed; immutable `dev.27` evidence cannot satisfy `dev.28`; CRED-UX4B/C workers/media and release actions remain unauthorized |
