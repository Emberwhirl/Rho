...
| `plans/active-2026-08-10-agpl-license-transition-spec.md` | active; LIC-1 implementation and independent review complete; `Emberwhirl` consent recorded; branch synchronized through upstream `main` `1b3f522` with `dev.30`, Issue #33, Rust 1.88/Resolver 3, and full affected local validation passed; `xuzhougeng` consent or authorized legal determination and future distribution acceptance remain open | prospective `AGPL-3.0-only` source-license boundary, synchronized repository metadata, contribution terms, third-party exclusions, and transition gates | keep the reviewed implementation in Draft; merge only after the remaining contributor permission or an authorized legal/ownership determination is recorded; future distribution requires a separately accepted candidate |
...
| `release/active-0.4.0-dev.30-candidate-checklist.md` | active source-candidate contract; Issue #33 implementation/review, exact identity synchronization, branch publication, Draft PR #34, and Issue reply complete; PR #24 dependency integrated at `f05315c`; branch refreshed through MSRV merge `9e0b36b` with complete local validation; final hosted source matrix pending | sole `0.4.0-dev.30` source/candidate identity and future exact-candidate, installed, MAC5, and publication evidence | source integration may proceed after exact hosted CI; no artifact, tag, installed acceptance, integration, or publication exists; `dev.29` evidence is non-composable; package versions and schema 12 remain unchanged |
...
| `plans/active-2026-08-11-workbench-focus-stability-repair-spec.md` | active; Issue #33 ISSUE-33-INTERACTION-1 source implementation, 46 compatible frontend contracts plus adjacent checks, and post-verification review complete; `0.4.0-dev.30` authorized; branch refreshed through upstream `main` `9e0b36b` with complete local validation; exact hosted source CI, browser/mock interaction, and installed Windows acceptance open | interaction-safe volatile refresh, explicit focus authority, activation preservation, and user-owned reading position across Project files/tabs, Agent, Runs, Problems, Plots/Outputs, Environment/Data Viewer, Git review, editor, Console, and Logs | WP2/WP3/UX4/Agent conversation/Environment retain data, execution, polling, selection, approval, and mutation truth; Issue #15 cursor advancement, Issue #18 modal predicate, and UX-FIX4 explicit Console focus preserved with bidirectional amendments; source merge requires hosted CI while manual gates remain separate; no schema, backend, credential, filesystem, or project authority |
