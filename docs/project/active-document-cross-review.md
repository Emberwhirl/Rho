...
| `release/active-0.4.0-dev.30-candidate-checklist.md` | active source-candidate contract; Issue #33 implementation/review, exact identity synchronization/source validation, branch publication, Draft PR #34, and Issue reply complete; PR #24 dependency integrated at `f05315c`, with refreshed-source validation pending | sole `0.4.0-dev.30` source/candidate identity and future exact-candidate, installed, MAC5, and publication evidence | refresh PR #34 against integrated `dev.29`, then complete exact-source review; no artifact, tag, installed acceptance, integration, or publication exists; `dev.29` evidence is non-composable; package versions and schema 12 remain unchanged |
