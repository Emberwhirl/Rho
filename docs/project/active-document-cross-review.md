...
| `release/active-0.4.0-dev.30-candidate-checklist.md` | active stacked source-candidate contract; Issue #33 implementation/review and exact identity synchronization/source validation complete; branch publication pending | sole `0.4.0-dev.30` source/candidate identity and future exact-candidate, installed, MAC5, and publication evidence | stacks exact PR #24 head and must not merge before #24; no artifact, Draft, tag, installed acceptance, integration, or publication exists; `dev.29` evidence is non-composable; package versions and schema 12 remain unchanged |
