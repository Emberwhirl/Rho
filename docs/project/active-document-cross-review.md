...
| `plans/active-2026-08-13-dev38-test-signed-prerelease-spec.md` | active authorized DEV38-SIGN1 contract; source integration and exact candidate/audit pass, while human installed acceptance, MAC5, publication, and update projection remain separate open gates | fresh `0.4.0-dev.38` identity, candidate-only Free Trial self-signed installer request/evidence, truthful public trust wording, exact installed ledger, and publication decision | preserve rehearsal as unsigned/review-only, never reuse FT-SIGN1 `dev.37` bytes, keep Foundation/production authority in SP-READY1, and do not publish before exact human acceptance plus MAC5 GO |
...
| `plans/active-2026-08-11-linux-appimage-support.md` | active authorized direction; LIN1-LIN4 source implementation present 2026-08-13; partial verification 2026-08-14 (LIN1/LIN3 fixture suites and `cargo fmt` pass; workspace suite runs with three pre-existing Linux update-manifest gaps in untouched `update.rs`; LIN2 build lanes and LIN5 remain unrun); LIN5 and the M3 release decision remain open | Linux x64 AppImage distribution, WebKitGTK 4.1 `AppRun` dependency check, Linux Ark bootstrap/sidecar, Linux R discovery, and local/hosted Linux build lanes | follow the macOS adapter template for configuration ownership, runtime manifest, R discovery, and evidence; keep Windows x64 and macOS arm64 behavior unchanged; no Linux distribution before the M3 gate; no `.deb`/`.rpm`, Linux arm64, or automatic Linux updates in this round |
...
| `release/active-0.4.0-dev.38-candidate-checklist.md` | active DEV38-SIGN1 ledger; exact source/artifact/audit evidence pass, automated macOS installed smoke is supporting-only, and human installed/MAC5/publication evidence remains open | sole `0.4.0-dev.38` identity, final candidate hashes, installed observations, acceptance asset, GO/NO-GO, and publication/update reconciliation | no `dev.37` evidence is composable; candidate mode requires fresh Free Trial signing evidence; publication remains NO-GO until exact human Windows/macOS acceptance and MAC5 GO |
