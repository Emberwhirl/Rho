...
| `release/active-0.4.0-dev.30-candidate-checklist.md` | active integrated source-candidate contract; Issue #33 implementation/review and exact identity synchronization complete; exact PR #34 head passed hosted run `31511253088` and integrated at `1b3f522`; owner authorized one exact current-main `DEV30-CANDIDATE-1` protected build/Draft dispatch on 2026-08-11; browser/mock and installed Windows/macOS acceptance open | sole `0.4.0-dev.30` source/candidate identity and future exact-candidate, installed, MAC5, and publication evidence | run one protected exact-main candidate build and record immutable evidence, then stop; no acceptance asset, installed acceptance, MAC5, or publication exists; Issue #33 remains open; `dev.29` evidence is non-composable; package versions and schema 12 remain unchanged |
