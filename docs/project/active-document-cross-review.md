...
| `plans/active-2026-08-05-macos-arm64-support-spec.md` | active; MAC1-MAC4 implementation/review complete; `0.4.0-dev.1` installed artifact rejected; bounded `0.4.0-dev.2` entitlement repair and replacement rehearsal passed; latest upstream `b5800ae` integrated at `0.4.0-dev.16`; MAC4-R3 local matrix and post-merge exact-commit rehearsal `31165265090` passed; Issue #4 reached historical replacement identity `0.4.0-dev.19`; Issue #6 now owns live identity `0.4.0-dev.20` without reusing earlier artifact evidence; exact local unsigned dev.20 DMG verification passes | Apple Silicon macOS 14+ platform adapters, Ark/R integration, Keychain extension, additive macOS update artifact, signed DMG handoff, repository-bound rehearsal lane, and async notarization orchestration | MAC4-R3 is closed at refreshed review-only exact-commit fork evidence; owner-installed dev.20 acceptance remains open; MAC5 remains unauthorized; full M3 remains open for authoritative candidate gates, macOS x64, and Linux x64 |
...
| `release/historical-0.4.0-dev.19-candidate-checklist.md` | historical superseded identity; CRED-UX4A-R1 local matrix, browser review, security review, and unsigned artifact verification passed before Issue #6 advanced the baseline | immutable `0.4.0-dev.19` source/artifact/hash and NO-GO record only | cannot authorize or satisfy any `0.4.0-dev.20` candidate, acceptance, MAC5, or publication row |
| `release/active-0.4.0-dev.20-candidate-checklist.md` | active Issue #6 replacement identity; PROBLEMS-AGENT-REPAIR-2 implementation, complete affected matrix, deterministic browser behavior review, and exact local unsigned artifact verification pass; installed acceptance, authoritative assets/draft, MAC5, and GO NOT RUN | sole live `0.4.0-dev.20` cross-platform candidate identity, future evidence binding, installed-acceptance ledger, and GO/NO-GO decision | authoritative candidate requires separate main-repository authorization; installed accessibility and MAC5/upload/publication remain separately gated |
...
| `plans/active-2026-08-07-problems-agent-repair-spec.md` | active; PROBLEMS-AGENT-REPAIR-1 historical partial implementation; owner authorized D3/R3 PROBLEMS-AGENT-REPAIR-2 correction 2026-08-08; implementation and affected automated/browser verification complete; exact local artifact and installed acceptance open | exact Workspace R expression diagnostics, schema-v10 project-scoped Problem ranges, bounded same-run context, and a read-only tool-capable Problems-to-Agent proposal flow | Runs/Workspace execution own diagnostic truth; BH3 migration rules own backup/rollback/reopen; CRED-UX4 owns the effective `agent.act` route and one credential; Agent file-edit owns proposal/Accept; no automatic execution, approval, save, fuzzy location, or file mutation authority |
...
`0.4.0-dev.19`. Issue #6 owns replacement identity `0.4.0-dev.20` under its
...
The active `0.4.0-dev.20` checklist inherits these unchanged automation gates,
not their results. PROBLEMS-AGENT-REPAIR-2 changes durable execution schema,
Agent routing and frontend behavior and therefore requires a new exact-source
validation, candidate, and installed-acceptance ledger; no earlier run, notarization
