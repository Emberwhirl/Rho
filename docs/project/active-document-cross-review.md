...
| `plans/active-2026-08-13-conditional-prerelease-policy-spec.md` | active authorized CPREL1 contract; source policy implementation in progress | fresh `0.4.0-dev.39` identity, schema-v2 actor-bound `CONDITIONAL_GO`, exact two-limit waiver, public disclosure, acceptance-aware update projection, and conditional publication decision | preserve ordinary GO compatibility and all artifact/security gates; conditional admission is allowlisted to dev.39/public-prerelease-only and cannot make either human observation pass |
...
| `release/active-0.4.0-dev.39-candidate-checklist.md` | active CPREL1 ledger; policy implementation authorized and all source/candidate/publication evidence initially open | sole `0.4.0-dev.39` identity, candidate hashes, exact conditional limitations, v2 acceptance asset, publication and update reconciliation | proceeds only through CPREL1 work-package gates; neither unrun human row may be represented as passed |
