...
| `release/active-0.4.0-dev.28-candidate-checklist.md` | active source-candidate contract; Issue #25 R3/R4 implementation, affected automation/browser review, independent review, and application identity synchronization pass; upstream integration remains | sole `0.4.0-dev.28` source/candidate identity and future exact-candidate, installed, MAC5, and publication evidence | no artifact, Draft, tag, acceptance, or publication exists; immutable `dev.27` evidence is non-composable; package versions and schema 12 remain unchanged |
