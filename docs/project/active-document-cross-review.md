...
| `plans/active-2026-08-11-signpath-application-readiness-spec.md` | active; SP-READY1 implementation, affected local automation, mock UI review, and post-test policy/security review complete; hosted validation, integration, external application, and production signing open | manual-only update admission, public privacy/security/code-signing policies, policy links, CODEOWNERS, and deterministic readiness enforcement | require exact-head and merged-main hosted matrices; do not create guessed SignPath policy/workflow configuration, construct a candidate, or claim Windows signing before external identifiers and approval exist |
...
| `release/active-0.4.0-dev.33-candidate-checklist.md` | active replacement source contract; CRED-UX3-R1 deterministic timeout-fixture repair, identity synchronization, complete local matrix, and post-test production-boundary review pass | sole `0.4.0-dev.33` source identity and any future exact candidate, installed, MAC5, publication, and updater evidence | push a scoped Draft PR and require exact-head hosted CI before integration; no candidate, installed acceptance, MAC5, publication, or updater mutation before its named gates |
