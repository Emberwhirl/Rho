...
| `plans/active-2026-08-10-rust-msrv-build-contract.md` | active; Issue #28 authorized `MSRV-1`; source implementation and local macOS acceptance complete; hosted matrix pending | Rust 1.88 workspace MSRV metadata, Resolver 3, non-packaging stable/MSRV native CI, locked candidate Rust validation, and deterministic policy enforcement | source slice may proceed to Draft PR; stop after all four hosted matrix outcomes; no candidate or release authority |
