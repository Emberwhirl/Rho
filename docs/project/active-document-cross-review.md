...
| `plans/active-2026-08-05-macos-arm64-support-spec.md` | active; MAC1-MAC4 implementation/review complete; `0.4.0-dev.1` installed artifact rejected; bounded `0.4.0-dev.2` entitlement repair locally verified; replacement hosted rehearsal and candidate draft NOT RUN | Apple Silicon macOS 14+ platform adapters, Ark/R integration, Keychain extension, additive macOS update artifact, signed DMG handoff, and repository-bound rehearsal lane | MAC4-R2 stops at review-only fork evidence; MAC5 remains unauthorized; full M3 remains open for authoritative candidate gates, macOS x64, and Linux x64 |
| `release/active-0.4.0-dev.2-candidate-checklist.md` | active; `0.4.0-dev.1` rehearsal rejected by installed official-R library validation; bounded `0.4.0-dev.2` repair locally verified 2026-08-06; replacement rehearsal, authoritative candidate assets, hosted draft, MAC5 acceptance, and GO all pending | sole replacement `0.4.0-dev.2` cross-platform candidate identity, rehearsal/candidate separation, evidence binding, installed-acceptance ledger, and GO/NO-GO decision | entitlement exception is exact and regression-gated; credentialed authoritative candidate may run only in the main repository; MAC5 acceptance/upload/publication requires separate authorization and an exact immutable draft |
...
| `plans/active-2026-08-05-outputs-viewer-spec.md` | active; OUTPUTS-VIEWER-1 implementation and automated/browser verification complete 2026-08-05; installed acceptance open | Outputs projection and bounded central inspection of Plot, exact Artifact HTML, Markdown buffers, and CSV/TSV files, including the isolated HTML security boundary | WP3/P2-3A/P2-3B retain Artifact and Render truth; PLOT-UX1 retains Plot history; UX4-AWS1 retains Agent work surfaces; remaining WS3/WS5 scope stays proposed |
...
| `plans/active-2026-08-05-system-credential-and-simple-llm-settings-spec.md` | active; CRED-UX1 implemented and automated/browser verified; MAC3 macOS Keychain adapter and isolated native smoke verified 2026-08-05; Windows installed acceptance open | shared credential semantics plus Windows system credential storage, Agent R secret injection, `.Renviron` compatibility fallback, and simplified LLM settings; macOS spec owns its adapter | implemented LLM V1 retains model/provider/capability/selection authority; Workspace R, Agent approval, project, history, and no-fallback boundaries unchanged; Windows installed credential-manager acceptance remains separate |
...
| `plans/active-2026-08-04-agent-execution-output-review-repair-spec.md` | active; AGENT-LOOP-1 implemented and automated/browser verified 2026-08-04; duplicate-Plot repair implemented and automated verified 2026-08-05; installed acceptance open | display-only path cleanup, direct Act execution instruction, Agent-first Outputs-to-Review loop, and single-execution duplicate Plot suppression over existing Plots and WP3 Artifacts | existing project/revision/approval/tool guards preserved; no new schema or authority; exact installed-candidate acceptance remains open |
...
replacement `0.4.0-dev.2` identity are owned by the D4 active checklist. MAC4
implemented parallel candidate
...
MAC4-R2 is the bounded installed-app repair lane jointly owned by the active
macOS specification and `0.4.0-dev.2` checklist. It may add only
`com.apple.security.cs.disable-library-validation` to the macOS signing plist,
validate that exact final signature on Rho and Ark, advance synchronized
application identity, and rerun fork rehearsal evidence. It does not authorize
