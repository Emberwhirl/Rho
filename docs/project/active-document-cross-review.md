...
| `plans/active-2026-08-10-windows-agent-r-script-launch-repair-spec.md` | active; Issue #2 ISSUE-2-AGENT-LAUNCH-1 launcher and aisdk-readiness repairs implemented; exact updated-main focused/full Rust validation and owner desktop Agent acceptance passed; pull-request review pending | Windows desktop Agent R script transport through a temporary UTF-8 `.R` file plus complete-Agent aisdk readiness admission | Windows startup retains R discovery and optional-runtime policy; Agent LLM configuration retains provider/credential/routing authority; Conversation concurrency retains turn identity, scheduling, cancellation, and persistence; no schema, frontend, focus behavior, approval, or release authority |
