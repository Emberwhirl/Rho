...
| `release/active-0.4.0-dev.32-candidate-checklist.md` | active replacement source-candidate contract; WS2-R1-R1 and RENAME-RECOVERY-R1 response-envelope correction, identity synchronization, local automation, and post-verification review pass | sole `0.4.0-dev.32` source identity and future exact candidate, installed, MAC5, publication, and updater evidence | no commit/PR integration, artifact, Draft, tag, installed acceptance, MAC5, publication, or updater mutation exists; `dev.31` evidence is non-composable; hosted source and downstream gates remain open |
