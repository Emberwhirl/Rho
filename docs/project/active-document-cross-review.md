...
| `plans/active-2026-08-05-system-credential-and-simple-llm-settings-spec.md` | active; CRED-UX1/2/3/4A and CRED-UX4A-R1 implementation verified; CRED-UX4A-R2 registered-runtime correction implemented; CRED-UX4A-R3 implementation/automated/screenshot/independent review complete 2026-08-10 with final interactive and installed acceptance open | shared native system-credential semantics, Model settings, bounded Provider discovery, three-layer Connection/Model/Capability routing, reviewed `aisdk.providers` integration, canonical registered runtime identity, and revision-bound Provider/model/optional-route/credential deletion | exact one-credential and no-fallback rules preserved; required `agent.chat` cannot be inferred or cascade-deleted; credential failure/metadata rollback remains fail-closed; `dev.27` cannot package this source-only slice; CRED-UX4B/C workers/media and release actions remain unauthorized |
