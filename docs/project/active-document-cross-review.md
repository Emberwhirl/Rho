...
| `release/active-0.4.0-dev.32-candidate-checklist.md` | active replacement source-candidate contract; WS2-R1-R1/RENAME-RECOVERY-R1 correction, identity synchronization, local validation/review, and exact implementation-head four-identity hosted source matrix pass | sole `0.4.0-dev.32` source identity and future exact candidate, installed, MAC5, publication, and updater evidence | upstream integration, artifact, Draft, tag, installed acceptance, MAC5, publication, and updater mutation remain open; `dev.31` evidence is non-composable; evidence-record commit needs exact-head CI |
