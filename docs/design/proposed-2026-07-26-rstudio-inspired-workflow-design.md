...
| WS3 object and artifact inspection | Partial: bounded viewers, Workspace-owned literal search, stable absolute-column sort, matched paging, page-size control, frozen identifier column, keyboard navigation, exact visible-page exports, and Artifact provenance are implemented | bounded tree navigation; richer type/missing-value presentation; broader plot zoom/format review; complete common inspection/provenance navigation; TanStack Table remains deferred rather than required for the current vanilla-table slice |
