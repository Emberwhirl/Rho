...
const expectedVersion = "0.4.0-dev.17";
...
assert.match(read("desktop/dist/index.html"), /styles\.css\?v=0\.4\.0-dev\.17/);
assert.match(read("desktop/dist/index.html"), /app\.js\?v=0\.4\.0-dev\.17/);
assert.ok(count(read("desktop/dist/app.js"), /0\.4\.0-dev\.17/g) >= 3, "Mock identity must be synchronized");

const localPackagePattern = /name = "rho-[^"]+"\r?\nversion = "([^"]+)"/g;
assert.deepEqual(
  [...'name = "rho-fixture"\r\nversion = "0.4.0-dev.17"'.matchAll(localPackagePattern)].map((match) => match[1]),
  [expectedVersion],
...
assert.match(read(".github/workflows/candidate-publish.yml"), /default: v0\.4\.0-dev\.17/);
