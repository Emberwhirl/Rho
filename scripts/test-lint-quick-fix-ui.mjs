...
assert.match(html, /id="editorCheckCodeButton"[^>]+title="Check the saved R file with lintr"[^>]+aria-label="Check code"/);
assert.match(html, /id="editorCheckCodeButton"[\s\S]*?#icon-check/);
assert.doesNotMatch(html, /id="lintCurrentFileButton"/);
assert.match(js, /editorCheckCodeButton"\)\.addEventListener\("click", lintCurrentFile\)/);
assert.match(js, /editorCheckCodeButton"\)\.disabled = documentActionsDisabled \|\| !activeDocument\(\)\?\.path\?\.toLowerCase\(\)\.endsWith\("\.r"\)/);
assert.match(js, /button\.setAttribute\("aria-busy", "true"\)/);
assert.match(js, /button\.removeAttribute\("aria-busy"\)/);
...
assert.match(js, /Save the active file before checking code so diagnostics match the saved source/);
assert.match(js, /if \(result\.error\) \{[\s\S]*rule: result\.notices\?\.includes\("provider_unavailable"\)/);
assert.match(js, /rule: "provider_exception"/);
...
assert.match(js, /The active project changed\. Check code again in this project\./);
...
assert.match(js, /finally \{\s*button\.removeAttribute\("aria-busy"\);\s*updateEditorChrome\(\);/);
assert.doesNotMatch(js, /Run Lint again|run Lint again/);
